...
class SCP03ChannelTest {
    ...
    fun `openSecureChannel throws when card cryptogram does not verify`() {
        ...
        // key-version byte (index 10) carries the expected 0x02 so parsing
        // proceeds; the computed cryptogram will not match the zero bytes, so
        // verification must reject.
        mock.respondWith(ByteArray(29) { 0x00 }.also { it[10] = 0x02 })
        val channel = newChannel(mock)
    ...
    fun `INITIALIZE UPDATE is the first APDU transmitted during open`() {
        val mock = MockBIBO()
        // Use a response guaranteed to fail cryptogram verification (valid
        // key-version byte, zeroed cryptogram), so we can observe the
        // INITIALIZE UPDATE command without dealing with EXTERNAL
        // AUTHENTICATE. This test asserts ordering, not crypto.
        mock.respondWith(ByteArray(29) { 0x00 }.also { it[10] = 0x02 })
        val channel = newChannel(mock)
