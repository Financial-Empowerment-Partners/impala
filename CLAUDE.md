...
**CLI conventions (lumencli/impalactl):** data on stdout, notices/prompts on stderr; exit 0/1/2; secrets never in argv (env var, no-echo prompt, or stdin); identifiers printed in full, never truncated; mainnet fund-moving commands require explicit confirmation, and the missing-memo guard's override is deliberately separate from `--yes`.
...
- `cargo test` (695+) runs with no Postgres/Redis: SQL is pinned by string/tripwire tests only, so schema-vs-query drift is a runtime failure class — double-check migrations against every query touching changed tables.
